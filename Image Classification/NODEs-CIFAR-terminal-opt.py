...
parser.add_argument('--save', type=str, default='./FashionMNIST_GPU_60_terminal_opt')
...
class ODEBlock(nn.Module):
    ...
    def forward(self, x):
        t0, t1 = nn.Parameter(torch.tensor([0.0]), requires_grad=False), self.integration_time
        # self.integration_time = self.integration_time.type_as(x)
        print('terminal time:')
        print(t1)
        # t_seq = torch.arange(t0.item(), t1.item(), self.dt).to(device)
        # t_seq = torch.cat((t_seq.to(device), t1.view(1).to(device)), dim=0)
        t_seq = torch.tensor([0, 1]).float()
        out = odeint(self.odefunc, x, t_seq, method = 'rk4', rtol=args.tol, atol=args.tol)
        a = out[-1]
...
if __name__ == '__main__':
    ...
    lr_fn = learning_rate_with_decay(
        args.batch_size, batch_denom=128, batches_per_epoch=batches_per_epoch, boundary_epochs=[10, 20, 40],
        decay_rates=[1, 0.1, 0.01, 0.001]
    )
    ...
    for itr in range(args.nepochs * batches_per_epoch):
        ...
        loss += torch.square(model._modules['7'].integration_time)
        ...
        if itr % batches_per_epoch == 0:
            with torch.no_grad():
                ...
                if val_acc > best_acc:
                    torch.save({'state_dict': model.state_dict(), 'args': args}, os.path.join(args.save, 'model_gpu_60_terminal_opt.pth'))
                    best_acc = val_acc
                logger.info(
